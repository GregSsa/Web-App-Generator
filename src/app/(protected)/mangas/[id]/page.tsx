...
export default async function MangaDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const currentNumber = Number(progress?.last_read_number ?? 0);
  ...
  return <div>
    <div className="flex flex-wrap items-start justify-between gap-4">
      ...
      <div className="flex flex-wrap gap-2"><CheckButton id={id} />{latestStored && <ExternalChapterLink chapterId={latestStored.id} href={latestStored.url}>Ouvrir le dernier chapitre</ExternalChapterLink>}<DeleteMangaButton mangaId={id} title={manga.title} /></div>
    </div>
    ...
    <div className="mt-6 grid gap-6 lg:grid-cols-[2fr_1fr]">
      <Card className="min-w-0"><CardHeader><CardTitle>Chapitres détectés</CardTitle></CardHeader><CardContent>
        ...
        {allNumbers.length ? <ScrollArea className="h-[36rem] pr-3"><div className="divide-y">{allNumbers.map((number) => { const chapter = storedByNumber.get(number); const inferredUrl = anchor ? deriveChapterUrl(anchor.url, Number(anchor.number_normalized), number) : null; const canOpenInferred = Boolean(inferredUrl && inferredUrl !== anchor?.url); const isRead = number <= currentNumber; return <div key={number} className="flex flex-wrap items-center justify-between gap-3 py-3"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><p className="font-medium">Chapitre {number}</p>{!chapter && <Badge variant="outline">Déduit</Badge>}<Badge variant={isRead ? "secondary" : "default"}>{isRead ? "Lu" : "Non lu"}</Badge></div>{chapter?.title && <p className="truncate text-sm text-muted-foreground">{chapter.title}</p>}</div><div className="flex gap-2"><form action={setProgressFromForm.bind(null, id)}><input type="hidden" name="chapterNumber" value={number} /><Button size="sm" variant={number === currentNumber ? "default" : "outline"}>Dernier lu</Button></form>{chapter ? <ExternalChapterLink chapterId={chapter.id} href={chapter.url}>Ouvrir</ExternalChapterLink> : canOpenInferred ? <Button size="sm" asChild><a href={inferredUrl!} target="_blank" rel="noopener noreferrer">Ouvrir</a></Button> : null}</div></div>; })}</div></ScrollArea> : <p className="py-8 text-center text-sm text-muted-foreground">Aucun chapitre enregistré.</p>}
      </CardContent></Card>
